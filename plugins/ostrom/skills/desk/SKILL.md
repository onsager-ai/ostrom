...
description: Read, lint, and decide mandate queue items. Use when the user types
  /ostrom:desk, asks what portfolio decisions are waiting, or asks to approve,
...
it. The dossier shape is the constitution subsystem's frozen escalation
...
change, use `scope_changes.entered` and `scope_changes.left` to show `/ostrom:desk`

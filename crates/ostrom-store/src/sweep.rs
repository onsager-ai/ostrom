...
/// Detect against retained state without loading policy, minting cached tokens,
/// emitting events, or invoking the generation writer. Live probes use gh's
/// existing authentication (including an explicitly supplied GH_TOKEN).
pub fn detect_sweep(
    paths: &OstromPaths,
    working_directory: &Path,
    fixture: Option<&Path>,
    since: Option<&str>,
) -> Result<Vec<(String, bool)>, SweepError> {
    let config = load_config(paths, working_directory)?;
    let state_path = match since {
        None | Some("current") => paths.sweep_state_file(),
        Some("previous") => paths.previous_sweep_dir().join("state.json"),
        Some(id) => {
            return Err(SweepError::State(format!(
                "unknown generation {id}; retained generations are current and previous"
            )));
        }
    };
    if since.is_some() && !state_path.is_file() {
        return Err(SweepError::State(format!(
    ...
    config
        ...
        .map(|project| {
            ...
            let etag = string_field(&previous, &["etag"]);
            if etag.is_empty() {
                return Ok((repo.to_owned(), true));
            }
            let probe = if let Some(fixture) = &fixture {
                let snapshot = fixture
                    .repositories
                    .iter()
                    .find(|snapshot| snapshot.repo == project.repo)
                    .expect("validated roster");
                Ok((
                    snapshot.issue_not_modified
                        && snapshot.issue_etag.as_deref() == Some(etag)
                        && snapshot.issues.is_empty(),
                    snapshot
                        .open_prs
                        .iter()
                        .chain(&snapshot.merged_prs)
                        .cloned()
                        .collect::<Vec<_>>(),
                    snapshot.default_branch.is_some()
                        || !snapshot.branches.is_empty()
                        || !snapshot.ci_runs.is_empty()
                        || snapshot.branch_read_degraded
                        || !snapshot.warnings.is_empty(),
                ))
            } else {
                detect_repository_sources(repo, &previous, &state)
            };
            // Failed or incomplete probes are fresh-work signals, never evidence of
            // no change. Detection stays read-only even when credentials are absent.
            let changed = probe.map_or(true, |(issues_unchanged, pulls, has_branch_sources)| {
                repository_changed(&previous, issues_unchanged, &pulls, has_branch_sources)
            });
            Ok((repo.to_owned(), changed))
        })
        .collect()
}

fn detect_repository_sources(
    repo: &str,
    previous: &Value,
    state: &Value,
) -> Result<(bool, Vec<Value>, bool), SweepError> {
    // An ETag belongs to the exact URL last swept. The last request's cursor
    // is previous_cursor, not the cursor advanced by that response.
    let since = if string_field(state, &["sweep_mode"]) == "incremental" {
        string_field(previous, &["previous_cursor"])
    } else {
        ""
    };
    let (_, _, issues_unchanged) = fetch_issues(repo, since, string_field(previous, &["etag"]))?;
    // A full sweep also reads closed issue updates. An unchanged open-issue
    // list cannot rule out a closure or an edit to an already closed issue.
    let cursor = string_field(previous, &["cursor"]);
    if cursor.is_empty() {
        return Ok((false, Vec::new(), true));
    }
    let (delta, _, _) = fetch_issues(repo, cursor, "")?;
    let issues_unchanged = issues_unchanged && delta.is_empty();
    let pulls = gh_json(&[
        "pr",
        "list",
        "--repo",
    ...
    let pulls = exhaustive_array(pulls, repo, "detect pull-request query")?;
    let branch = gh_json(&["repo", "view", repo, "--json", "defaultBranchRef"])?;
    let has_branch_sources = branch.get("defaultBranchRef") != Some(&Value::Null);
    Ok((issues_unchanged, pulls, has_branch_sources))
}

fn repository_changed(
    previous: &Value,
    issues_unchanged: bool,
    pulls: &[Value],
    has_branch_sources: bool,
) -> bool {
    if !issues_unchanged {
        return true;
    }
    let records = previous.get("records").and_then(Value::as_object);
    for pull in pulls {
        let number = number_field(pull, &["number"]);
        let old = records.and_then(|records| {
            records.values().find(|record| {
                number_field(record, &["number"]) == number
                    && string_field(record, &["type"]) == "pr"
            })
        });
        if old
            .is_none_or(|old| string_field(old, &["updated"]) != string_field(pull, &["updatedAt"]))
        {
            return true;
        }
    }
    // PR updatedAt cannot certify check/status rollups or changed files;
    // default-branch CI and branch tips have no stored conditional baseline.
    // Issue relationship reads likewise have no ETag. Those sources cannot be
    // cheaply proven unchanged with the existing record format, so choose the
    // safe direction. Only a fully checked, empty repository can be unchanged.
    // A fixture follows this same rule; it never grants stronger freshness.
    has_branch_sources || !pulls.is_empty() || records.is_none_or(|records| !records.is_empty())
}

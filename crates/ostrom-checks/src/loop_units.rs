...
pub fn generate_loop_units(manifest: &PolicyManifest) -> Result<Vec<LoopUnit>, LoopUnitError> {
    let mut units = Vec::with_capacity(manifest.loops.len() * 2 + 2);
    if !manifest.loops.is_empty() {
        ...
        units.push(LoopUnit {
            ...
            contents: render_reconciler_timer(),
        });
    }
...
fn render_reconciler_timer() -> String {
    // Five minutes keeps normal observation delay small and leaves ample room
    // inside the reconciler's two-hour stale-slot bound if several firings are
    // missed. OnBootSec also gives a newly started user session a prompt pass.
    format!(
...
mod tests {
    ...
    use super::{
        RECONCILER_SERVICE, RECONCILER_TIMER, check_loop_units_drift, generate_loop_units,

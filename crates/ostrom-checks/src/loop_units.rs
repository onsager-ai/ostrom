...
pub fn generate_loop_units(manifest: &PolicyManifest) -> Result<Vec<LoopUnit>, LoopUnitError> {
    let mut units = Vec::with_capacity(manifest.loops.len() * 2);
...
fn managed_unit_name(name: &str) -> bool {
    name.starts_with(UNIT_PREFIX) && (name.ends_with(".service") || name.ends_with(".timer"))
}
...
mod tests {
    ...
    use super::{
        check_loop_units_drift, generate_loop_units, loop_execstart_is_not_shell, render_loop_units,
    };

...
fn check_run_records_verdicts_and_isolates_a_timeout() {
    ...
    assert!(
        String::from_utf8_lossy(&output.stdout)
            .contains("ostrom check run: 2 passed; 1 failed; 1 faulted")
    );
    ...
    assert_eq!(receipt("c-timeout").verdict, None);
    assert_eq!(receipt("c-timeout").error.as_deref(), Some("cmd_timeout"));
